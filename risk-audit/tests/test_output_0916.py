...
def file_record(path):
    ...
    aliases = load_pack(ROOT / 'risk-audit/rulepacks/releases/1.8.0')['field_aliases']
...
def test_missing_incompatible_sheet_is_still_reported(tmp_path, registry, hidden_incompatible):
    ...
    book.save(path)
    pack = load_pack(ROOT / 'risk-audit/rulepacks/releases/1.9.7')
...
def test_new_pack_audit_and_repeat_rebuild_opinions(tmp_path):
    ...
    active_pack = ROOT / 'risk-audit/rulepacks/releases/1.9.13'

    result = audit(source.parent, tmp_path / 'output', active_pack, ROOT / 'reference-data/会计主体清单20260907.xlsx', ROOT, tmp_path / 'runs', scope_file=scope)
    ...
    second = audit(source.parent, tmp_path / 'output', active_pack, ROOT / 'reference-data/会计主体清单20260907.xlsx', ROOT, tmp_path / 'runs', scope_file=scope)
...
def test_interrupted_run_ownership_replaces_manual_edit_on_retry(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match='模拟汇总阶段中断'):
        audit_runner.audit(source.parent, tmp_path / 'output', active_pack,
                           ROOT / 'reference-data/会计主体清单20260907.xlsx', ROOT, tmp_path / 'runs', scope_file=scope)
...
def test_audit_skips_dot_prefixed_temporary_file(tmp_path):
    ...
    result = audit(input_root, tmp_path / 'output', ROOT / 'risk-audit/rulepacks/releases/1.9.13',
                   ROOT / 'reference-data/会计主体清单20260907.xlsx', ROOT, tmp_path / 'runs', scope_file=scope)
