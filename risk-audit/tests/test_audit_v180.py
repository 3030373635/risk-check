...
def test_confirmed_pack_full_audit_rebuilds_opinions(tmp_path):
    ...
    result = audit(input_root, tmp_path / 'output', ROOT / 'risk-audit/rulepacks/releases/1.8.0', ROOT / 'reference-data/会计主体清单20260907.xlsx', ROOT, tmp_path / 'runs', scope_file=scope)
...
def test_configuration_rejects_invalid_responsibility_patterns():
    ...
    pack = deepcopy(load_pack(ROOT / 'risk-audit/rulepacks/releases/1.8.0'))
