...
def test_windows_workflow_installs_project_without_editable_mode() -> None:
    """Windows 必须用普通 wheel 安装项目；无参数，避免在 .pth 中固化源码路径。"""
    workflow = read_workflow()
    install_step = workflow.split("- name: 安装构建与测试依赖", 1)[1].split("\n      - name:", 1)[0]

    # editable 安装会把构建机的绝对源码路径写入 Windows .pth 文件。
    assert not re.search(r"(?<!\S)(?:-e|--editable)(?:\s|=)", install_step)
    assert r".\risk-audit[web,test]" in install_step
...
def test_web_test_step_runs_from_auditor_root(workflow_reader) -> None:
    ...
    assert "working-directory: risk-audit" in test_step
