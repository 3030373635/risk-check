"""按《风控矩阵0916-6》生成、验证并激活 v1.9.6 规则包。"""
...
BASE_VERSION = "1.9.5"
DRAFT_NAME = "rules-0916-6-1.9.6"
TARGET_VERSION = "1.9.6"
SOURCE_NAME = "风控矩阵0916-6.docx"
SOURCE_REFERENCE = f"{SOURCE_NAME} 有效正文（接受新增、排除修订删除）"
RETAINED_RULE_5_CHECKS = {
    "department_specific",
    "position_specific",
    "person_required",
}
...
def update_rule_5(rules_dir: Path) -> None:
    """收窄第5条检查范围；rules_dir 为草稿中的规则配置目录。"""
    rule_path = rules_dir / "R05.json"
    rule = json.loads(rule_path.read_text(encoding="utf-8"))
    rule["checks"] = [
        check for check in rule["checks"]
        if check["check_id"] in RETAINED_RULE_5_CHECKS
    ]
    person_check = next(
        check for check in rule["checks"]
        if check["check_id"] == "person_required"
    )
    # 0916-6要求按“包含关键词”判断，姓名中带“待定”也必须提示具体到人员。
    person_check["params"]["placeholder_patterns"] = [r"等|…|\.{3}|待定"]
    rule.update(
        revision=rule["revision"] + 1,
        title="岗位清单中部门、岗位名称和人员姓名",
    )
    write_json(rule_path, rule)


def disable_responsibility_coverage(rules_dir: Path) -> None:
    """停用已删除的矩阵责任覆盖检查；rules_dir 为草稿中的规则配置目录。"""
    rule_path = rules_dir / "R05b.json"
    rule = json.loads(rule_path.read_text(encoding="utf-8"))
    rule.update(
        enabled=False,
        revision=rule["revision"] + 1,
        disabled_reason="0916-6已删除第5条矩阵责任主体覆盖岗位清单的要求",
    )
    write_json(rule_path, rule)


def update_draft(draft: Path, source: Path) -> None:
    """同步0916-6规则草稿；draft 为草稿目录，source 为冻结来源文档。"""
    rules_dir = draft / "rules"
    update_rule_5(rules_dir)
    disable_responsibility_coverage(rules_dir)
    ...
    manifest.update(
        ...
        engine_compatibility=">=1.9.6,<2.0.0",
        audit_as_of="2026-09-16",
        title="风控矩阵0916-6第5条更新规则",
...
def build_release(store: RulePackStore, source: Path) -> tuple[Path, Path]:
    """生成并校验1.9.6；store 为规则仓库，source 为0916-6来源文档。"""
...
def verify_existing_release(store: RulePackStore, source: Path) -> Path:
    """重建并核对现有产物；store 为正式仓库，source 为0916-6来源文档。"""
    ...
    if not actual_draft.is_dir() or not actual_release.is_dir():
        raise RuntimeError("1.9.6草稿与发布包必须同时存在")
    with tempfile.TemporaryDirectory(prefix="risk-audit-rules-0916-6-") as temporary_directory:
...
def source_document(verify_only: bool) -> Path:
    ...
    # 规则来源随发布包冻结，后续复验不再依赖桌面附件位置。
    shutil.copy2(external, destination)
    return destination


def main() -> None:
    """生成或复验1.9.6；--verify-only 只验证产物，不修改激活版本。"""
    parser = argparse.ArgumentParser(description="生成或复验1.9.6规则包")
