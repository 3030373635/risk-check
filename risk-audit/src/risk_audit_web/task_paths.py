...
MAX_OUTPUT_DIRECTORY_STEM_LENGTH = 120
...
def sanitize_directory_name(raw_name: str) -> str:
    ...
    # 为时间戳、递增后缀和 Windows 完整路径预留空间。
    return (sanitized or "审核任务")[:MAX_OUTPUT_DIRECTORY_STEM_LENGTH].rstrip(" .") or "审核任务"
...
def validate_task_paths(input_root: Path, output_root: Path) -> None:
    ...
    managed_outputs_root = output_resolved.parent
    if (
        input_resolved == output_resolved
        or input_resolved in output_resolved.parents
        or output_resolved in input_resolved.parents
        or input_resolved == managed_outputs_root
        or managed_outputs_root in input_resolved.parents
    ):
