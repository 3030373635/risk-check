...
def test_sanitize_directory_name_replaces_only_windows_invalid_parts(
    ...
    from risk_audit_desktop.task_paths import sanitize_directory_name
...
def test_portable_paths_are_anchored_to_executable_directory(tmp_path: Path) -> None:
    """程序目录定位不得依赖当前工作目录。"""
...
def test_validate_task_paths_rejects_overlap(tmp_path: Path, relation: str) -> None:
    ...
    from risk_audit_desktop.task_paths import PathValidationError, validate_task_paths
...
def test_reserve_output_path_creates_unique_directory(tmp_path: Path) -> None:
    ...
    from risk_audit_desktop.task_paths import reserve_output_path
