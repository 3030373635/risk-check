...
def _copy_runtime_resources(project_root: Path, auditor_root: Path, distribution_root: Path) -> None:
    ...
    source_materials = project_root / "审核"
    entity_name = "会计主体清单20260907.xlsx"
    entity_file = source_materials / entity_name
    if not entity_file.is_file():
        raise BuildError(f"会计主体清单缺失：{entity_file}")
    baseline_target = distribution_root / "runtime/resources/baselines/审核"
    baseline_target.mkdir(parents=True)
    for path in sorted(source_materials.rglob("*"), key=lambda item: item.as_posix()):
        if not path.is_file() or path.name in {".DS_Store", entity_name}:
            continue
        target = baseline_target / path.relative_to(source_materials)
...
def _locked_libreoffice_version(inputs: BuildInputs) -> str:
    ...
    source_manifest = inputs.project_root / "审核器/packaging/runtime-sources.json"
    try:
...
def assemble_distribution(inputs: BuildInputs) -> Path:
    ...
    auditor_root = inputs.project_root / "审核器"
