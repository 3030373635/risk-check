...
def update_draft(draft: Path) -> None:
    """修正新增规则的对外编号；draft 为从 1.9.4 复制的规则草稿目录。"""
    rules_dir = draft / "rules"
    for filename, (display_code, clause_number) in RULE_NUMBER_UPDATES.items():
        rule_path = rules_dir / filename
        rule = json.loads(rule_path.read_text(encoding="utf-8"))
        rule["display_code"] = display_code
        rule["revision"] += 1
        for check in rule["checks"]:
            # 意见正文仍由 advice_v2 生成，此处只修正条款前缀。
            check["message"] = {
                kind: f"【{clause_number}】{{advice_v2}}"
    ...
    manifest.update(
        ...
        engine_compatibility=">=1.9.5,<2.0.0",
        title="风控矩阵第16、17条编号修复规则",
    )
    write_json(manifest_path, manifest)


def build_release(store: RulePackStore) -> tuple[Path, Path]:
    """生成并校验 1.9.5 规则包；store 为规则仓库。"""
...
def verify_existing_release(store: RulePackStore) -> Path:
    """重建并校验已发布产物；store 为正式规则仓库。"""
    ...
    if not actual_draft.is_dir() or not actual_release.is_dir():
        raise RuntimeError("1.9.5 草稿与发布包必须同时存在")
    with tempfile.TemporaryDirectory(prefix="risk-audit-rule-numbers-") as temporary_directory:
        ...
        generated_draft, generated_release = build_release(temporary_store)
...
def main() -> None:
    """生成或复验 1.9.5；--verify-only 表示只读校验且不切换激活版本。"""
    parser = argparse.ArgumentParser(description="生成或复验 1.9.5 规则包")
    ...
    if draft_exists or release_exists:
        release = verify_existing_release(store)
        verified = True
    else:
        _, release = build_release(store)
        verified = False
