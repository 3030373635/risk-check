...
def update_draft(draft: Path, source: Path) -> None:
    ...
    manifest.update(
        ...
        engine_compatibility=">=1.9.13,<2.0.0",
...
def build_release(store: RulePackStore, source: Path) -> tuple[Path, Path]:
    """生成并校验1.9.13；store 为规则仓库，source 为冻结来源文档。"""
...
def verify_existing_release(store: RulePackStore, source: Path) -> Path:
    ...
    if not actual_draft.is_dir() or not actual_release.is_dir():
        raise RuntimeError("1.9.13草稿与发布包必须同时存在")
    with tempfile.TemporaryDirectory(prefix="risk-audit-rules-0917-9-") as temporary_directory:
...
def main() -> None:
    """生成或复验1.9.13；--verify-only 只校验产物，不修改激活版本。"""
    parser = argparse.ArgumentParser(description="生成或复验1.9.13规则包")
