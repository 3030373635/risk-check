"""按《0920-2》生成、验证并激活 v1.9.16 规则修复包。"""
...
BASE_VERSION = "1.9.15"
DRAFT_NAME = "rules-0920-2-fix-1.9.16"
TARGET_VERSION = "1.9.16"
SOURCE_NAME = "0920-2.docx"
...
def add_applicability_rule(rules_dir: Path) -> None:
    """新增矩阵适用性联动规则；rules_dir 为新规则包的规则目录。"""
    rule = {
        "checks": [
            {
...
def update_draft(draft: Path, source: Path) -> None:
    """同步0920-2规则草稿；draft 为草稿目录，source 为冻结来源文档。"""
    ...
    manifest.update(
        ...
        engine_compatibility=">=1.9.16,<2.0.0",
        audit_as_of="2026-09-20",
        title="0920-2第16条意见编号修复包",
...
def build_release(store: RulePackStore, source: Path) -> tuple[Path, Path]:
    """生成并校验1.9.16；store 为规则仓库，source 为冻结来源文档。"""
...
def verify_existing_release(store: RulePackStore, source: Path) -> Path:
    ...
    if not actual_draft.is_dir() or not actual_release.is_dir():
        raise RuntimeError("1.9.16草稿与发布包必须同时存在")
    with tempfile.TemporaryDirectory(prefix="risk-audit-rules-0920-2-") as temporary_directory:
...
def main() -> None:
    """生成或复验1.9.16；--verify-only 只校验产物，不修改激活版本。"""
    parser = argparse.ArgumentParser(description="生成或复验1.9.16规则包")
